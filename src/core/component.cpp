...
void Component::setInputs(const std::vector<Bus>& newInputs)
{
  ...
  for (const auto& [index, bus] : silicon::views::enumerate(newInputs)) {
    setInput(index, bus);
  }
}
...
{
  for (const auto [index, bus] : silicon::views::enumerate(this->outputs)) {
  ...
  for (const auto [index, bus] : silicon::views::enumerate(this->inputs)) {
    const auto                  busSize = bus.size();
    const std::vector<Wire_ptr> wires(busSize);
